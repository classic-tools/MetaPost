...
@y
@d banner=='This is MetaPost, C Version 0.631' {printed when \MP\ starts}
@z
...
@z
@x %% file_name_size pool_name ps_tab_name %%
...
@y
@ The following program does the required initialization
and also retrieves a possible command line and recognizes |'-I'|
and |'-T'| switches at the beginning of the command line.
@^system dependencies@>
...
begin
    ...
    if argc > 1 then begin
		...
		decr(k);
            if (k = 2) and (arg[1] = '-') and (last = first) then
		if arg[2] = 'I' then@\
		    begin @+init mem_ident := " (INIMP)"; goto continue;
			tini@; @+end
		else if arg[2] = 'T' then
		    begin troff_mode := true; goto continue; end;
            while (j <= k) do begin
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [Unique to MetaPost] Path selector for |a_open_in| of mpx file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
@x
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [48.1184] inimp should not set mem_ident until the -I flag is parsed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
@x
mem_ident:=0;
...
@ In the C version we leave |mem_ident| equal to zero until we see that
the user has actually requested \.{inimp} via the `\.{-I}' flag.  This
allows \.{inimp} to be used as the production version so that only one
executable \.{mp} is needed.
@z
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [48.1199] `eof' here means `have we previously encountered the
